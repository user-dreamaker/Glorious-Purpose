...
struct __attribute__((packed)) TrainerMonFull
{
    u8 lvl;
    u16 species;
    u16 heldItem;
...
#define TRAINER_PARTY_FULL(party) { .Full = party }, .partySize = ARRAY_COUNT(party), .partyFlags = F_TRAINER_PARTY_FULL
...
union TrainerMonPtr
{
    ...
    const struct TrainerMonFull *Full;
};
...
struct BattleStruct
{
    ...
    u8 friskAnnounced[MAX_BATTLERS_COUNT];
    u8 padding_1E8[0x14];
}; // size == 0x200 bytes
...
#define IS_TYPE_STATUS(move)(move.category == CATEGORY_STATUS)
