...
u8 GetCurrentLevelCap(void)
{
    ...
    if (count >= NUM_BADGES)
        return sLevelCapsByBadge[NUM_BADGES - 1];
    return sLevelCapsByBadge[count];
}
...
bool8 CanUseRareCandy(struct Pokemon *mon)
{
    ...
    if (GetMonData(mon, MON_DATA_LEVEL) >= (u16)(cap + 1))
        return FALSE;
    return TRUE;
}

bool8 IsMonAtLevelCap(u8 level)
{
    if (IsLevelCapMaxed())
        return FALSE;
...
bool8 WouldExceedLevelCap(u8 curLevel)
{
    if (IsLevelCapMaxed())
        return FALSE;
    return (curLevel + 1) > GetCurrentLevelCap();
}

bool8 IsMonBlockedFromExp(struct Pokemon *mon)
{
    return IsMonAtLevelCap(GetMonData(mon, MON_DATA_LEVEL, NULL));
}
