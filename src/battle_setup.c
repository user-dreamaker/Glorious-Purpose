...
{
    CreateMaleMon(&gEnemyParty[0], SPECIES_MEW, 5);
    LockPlayerFieldControls();
...
void StartMarowakBattle(void)
{
    ...
    if (CheckBagHasItem(ITEM_SILPH_SCOPE, 1))
    {
        ...
        CreateMonWithGenderNatureLetter(gEnemyParty, SPECIES_MAROWAK, 33, 31, MON_FEMALE, NATURE_ADAMANT, 0);
    }
    else
    {
        gBattleTypeFlags = BATTLE_TYPE_GHOST;
...
static u8 GetSumOfEnemyPartyLevel(u16 opponentId, u8 numMons)
{
    ...
    case F_TRAINER_PARTY_FULL:
        {
            const struct TrainerMonFull *party;

            party = gTrainers[opponentId].party.Full;
            for (i = 0; i < count; ++i)
                sum += GetDynamicLevel(party[i].lvl);
        }
        break;
