...
void CalculateMonStats(struct Pokemon *mon)
{
    ...
    if (!useCustomStats)
    {
        ...
        {
            u16 advanced = ChaseStatChainForMon(mon, overrideSpecies);
            if (advanced != SPECIES_NONE && advanced < NUM_SPECIES)
                overrideSpecies = advanced;
        }
        if (overrideSpecies != STAT_OVERRIDE_NONE)
...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    if (type == TYPE_BUG && attacker->ability == ABILITY_SWARM && attacker->hp <= (attacker->maxHP / 3))
        gBattleMovePower = (150 * gBattleMovePower) / 100;
    if (attacker->ability == ABILITY_IRON_FIST && gBattleMoves[move].flags & FLAG_PUNCHING_MOVE)
        gBattleMovePower = (150 * gBattleMovePower) / 100;
    ...
    if (attacker->ability == ABILITY_RECKLESS && move != MOVE_STRUGGLE && (gBattleMoves[move].effect == EFFECT_RECOIL || gBattleMoves[move].effect == EFFECT_DOUBLE_EDGE || gBattleMoves[move].effect == EFFECT_VOLT_TACKLE))
        gBattleMovePower = (150 * gBattleMovePower) / 100;
    if (move == MOVE_KNOCK_OFF && defender->item != ITEM_NONE && defender->ability != ABILITY_STICKY_HOLD)
        gBattleMovePower = (150 * gBattleMovePower) / 100;
    ...
    // Sandstorm raises the Special Defense of all Rock-type Pokémon by 50%.
    if (WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_SANDSTORM) && IS_BATTLER_OF_TYPE(battlerIdDef, TYPE_ROCK))
        spDefense = (15 * spDefense) / 10;
	
    // Hail raises the Defense of all Ice-type Pokémon by 50%.
