...
static const u16 sProtectSuccessRates[] = {USHRT_MAX, USHRT_MAX / 3, USHRT_MAX / 6, USHRT_MAX / 12};
...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        if (gBattleMons[gBattlerTarget].ability == ABILITY_TANGLED_FEET && (gBattleMons[gBattlerTarget].status2 & STATUS2_CONFUSION))
            calc = calc / 2; // 0.5 tangled feet loss
        ...
        while (TYPE_EFFECT_ATK_TYPE(i) != TYPE_ENDTABLE)
        {
            if (TYPE_EFFECT_ATK_TYPE(i) == TYPE_FORESIGHT)
            {
                if (gBattleMons[gBattlerTarget].status2 & STATUS2_FORESIGHT)
                    break;
                if (gBattleMons[gBattlerAttacker].ability == ABILITY_SCRAPPY && (moveType == TYPE_NORMAL || moveType == TYPE_FIGHTING))
                    break;
                i += 3;
                continue;
            }
...
u8 TypeCalc(u16 move, u8 attacker, u8 defender)
{
        ...
        while (TYPE_EFFECT_ATK_TYPE(i) != TYPE_ENDTABLE)
        {
            if (TYPE_EFFECT_ATK_TYPE(i) == TYPE_FORESIGHT)
            {
                if (gBattleMons[defender].status2 & STATUS2_FORESIGHT)
                    break;
                if (gBattleMons[attacker].ability == ABILITY_SCRAPPY && (moveType == TYPE_NORMAL || moveType == TYPE_FIGHTING))
                    break;
                i += 3;
                continue;
            }
    ...
    if ((gBattleMons[defender].ability == ABILITY_FILTER || gBattleMons[defender].ability == ABILITY_SOLID_ROCK)
     && (flags & MOVE_RESULT_SUPER_EFFECTIVE))
    {
        gBattleMoveDamage = gBattleMoveDamage * 3 / 4;
        flags &= ~MOVE_RESULT_SUPER_EFFECTIVE;
...
u8 AI_TypeCalc(u16 move, u16 targetSpecies, u8 targetAbility)
{
    ...
    else if (targetAbility == ABILITY_MOTOR_DRIVE && moveType == TYPE_ELECTRIC)
    {
        flags = MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE;
    }
    else
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_WRAP:
                ...
                {
                    gBattleMons[gEffectBattler].status2 |= STATUS2_WRAPPED_TURN((Random() & 1) + 5); // 4-5 turns
...
static void Cmd_getexp(void)
{
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else if (IsMonAtLevelCap(GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL)))
            {
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP)
             ...
             && !IsMonAtLevelCap(GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL)))
            {
    ...
    case 4: // lvl up if necessary
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (gBattleBufferB[gActiveBattler][0] == CONTROLLER_TWORETURNVALUES && gBattleBufferB[gActiveBattler][1] == RET_VALUE_LEVELED_UP)
            {
                ...
                if (IsMonAtLevelCap(GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL)))
                    gBattleMoveDamage = 0;
    ...
    do
    {
        ...
        case MOVEEND_DEFROST: // defrosting check
            if (gBattleMons[gBattlerTarget].status1 & STATUS1_FREEZE
                && gBattleMons[gBattlerTarget].hp != 0
                && gBattlerAttacker != gBattlerTarget
                && gSpecialStatuses[gBattlerTarget].dmg
                && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                && moveType == TYPE_FIRE)
            {
            ...
            case 0:
                {
                    ...
                    u8 lvl = party1[gTrainers[gTrainerBattleOpponent_A].partySize - 1].lvl;
            ...
            case F_TRAINER_PARTY_CUSTOM_MOVESET:
                {
                    ...
                    u8 lvl = party2[gTrainers[gTrainerBattleOpponent_A].partySize - 1].lvl;
                    lastMonLevel = lvl == 0 ? GetPlayerPartyHighestLevel() : lvl;
                }
                break;
            case F_TRAINER_PARTY_HELD_ITEM:
                {
                    ...
                    u8 lvl = party3[gTrainers[gTrainerBattleOpponent_A].partySize - 1].lvl;
                    lastMonLevel = lvl == 0 ? GetPlayerPartyHighestLevel() : lvl;
                }
                break;
            case (F_TRAINER_PARTY_CUSTOM_MOVESET | F_TRAINER_PARTY_HELD_ITEM):
                {
                    ...
                    lvl = party4[gTrainers[gTrainerBattleOpponent_A].partySize - 1].lvl;
                    lastMonLevel = lvl == 0 ? GetPlayerPartyHighestLevel() : lvl;
                }
                break;
            }
...
static u8 ChangeStatBuffs(s8 statValue, u8 statId, u8 flags, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else if (gBattleMons[gActiveBattler].ability == ABILITY_ILLUMINATE
                 && !certain && statId == STAT_ACC)
        {
            if (flags == STAT_CHANGE_ALLOW_PTR)
            {
